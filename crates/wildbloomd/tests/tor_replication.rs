...
const TOR_BOOTSTRAP_TIMEOUT: Duration = Duration::from_secs(300);
const READY_TIMEOUT: Duration = Duration::from_secs(330);
...
async fn two_nodes_replicate_repair_and_preserve_the_onion_identity() {
    ...
    let mirrored = client
        .put(format!("http://127.0.0.1:{second_port}/mirror"))
        .header("content-type", "application/json")
        .header(
            "authorization",
            authorization(&keys, "upload", &hash, &second_onion),
        )
